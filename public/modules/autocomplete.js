...
const fetchSuggestions = async (input)  => {
    const response = await fetch(`https://nominatim.openstreetmap.org/search.php?q=${input}&format=jsonv2`);
...
const renderLocations = async (e, latInput, lngInput) => {
    ...
    suggestions.innerHTML = locations
                                ...
                                .map(location => `
<li>
    <a class="dropdown-item" href="#" data-lat="${location.lat}" data-lng="${location.lon}">
    <img src="/icons/map-pin.svg" alt=""> ${location.display_name}</a>
</li>
     `).join('')
}
